...
if grep "authentication" ${PRESTO_PARCEL_PATH}/etc/kerberos.config | grep "true" >/dev/null; then
  ...
  {
  echo 'hive.metastore.authentication.type=KERBEROS'
  echo "hive.metastore.service.principal=hive/${metastore}@${realm}"
  echo "hive.metastore.client.principal=presto/$(hostname)@${realm}"
  echo "hive.metastore.client.keytab=${PRESTO_PARCEL_PATH}/etc/presto.keytab"
  echo 'hive.hdfs.authentication.type=KERBEROS'
  echo 'hive.hdfs.impersonation.enabled=true'
  echo "hive.hdfs.presto.principal=presto/$(hostname)@${realm}"
  echo "hive.hdfs.presto.keytab=${PRESTO_PARCEL_PATH}/etc/presto.keytab"
   }>>${PRESTO_PARCEL_PATH}/etc/catalog/hive.properties
fi

if grep "client" ${PRESTO_PARCEL_PATH}/etc/kerberos.config | grep "true" >/dev/null; then
  {
  echo 'http-server.authentication.type=KERBEROS'
  echo 'http.server.authentication.krb5.service-name=presto'
  echo "http.server.authentication.krb5.keytab=${PRESTO_PARCEL_PATH}/etc/presto.keytab"
  echo 'http.authentication.krb5.config=/etc/krb5.conf'
  cat ${PRESTO_PARCEL_PATH}/etc/client_kerberos.config
  }>>${PRESTO_PARCEL_PATH}/etc/config.properties
  {
    echo 'access-control.name=file'
    echo 'security.config-file=etc/rules.json'
  } >${PRESTO_PARCEL_PATH}/etc/access-control.properties
else
  echo 'access-control.name=allow-all' >${PRESTO_PARCEL_PATH}/etc/access-control.properties
fi

if grep "hive_auth" ${PRESTO_PARCEL_PATH}/etc/kerberos.config | grep "true" >/dev/null; then
  cp -f "${CONF_DIR}"/etc/hive_auth.config ${PRESTO_PARCEL_PATH}/etc/
  {
    echo 'security.refresh-period=15s'
    echo 'hive.security=file'
  }>>${PRESTO_PARCEL_PATH}/etc/catalog/hive.properties
