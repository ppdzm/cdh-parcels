...
if [ "${NODE_TYPE}" = "master" ]; then
  ...
  if [ "${RPC_ADDRESS}" = "" ]; then
    ...
    RPC_ADDRESS=$(grep "jobmanager.rpc.address:" "${CONF_DIR}"/flink-conf.properties)
    echo "RPC_ADDRESS: ${RPC_ADDRESS}"
  fi
fi
HIGH_MODE=$(grep "high-availability:" "${CONF_DIR}"/flink-conf.properties)
...
HIGH_ZK_QUORUM=$(grep "high-availability.zookeeper.quorum:" "${CONF_DIR}"/flink-conf.properties)
