...
readFromConfig() {
  ...
  value=$(sed -n "s/^[ ]*${key}[ ]*: \([^#]*\).*$/\1/p" "${configFile}" | sed "s/^ *//;s/ *$//" | tail -n 1)
  local value
...
runBashJavaUtilsCmd() {
  ...
  output=$("${JAVA_RUN}" -classpath "${class_path}" org.apache.flink.runtime.util.bash.BashJavaUtils "${cmd}" --configDir "${conf_dir}" "${dynamic_args}" 2>&1 | tail -n 1000)
  local output
  if [[ $? -ne 0 ]]; then
...
parseResourceParamsAndExportLogs() {
  ...
  DYNAMIC_PARAMETERS=$(IFS=" " echo "${params_output}" | tail -n1)
  export DYNAMIC_PARAMETERS
